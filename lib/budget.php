...
class Budget
{
    ...
    public function GetCategoryTotal($project_id, $cat_id)
    {
        $conn = new MysqlConnect();
        $stmt = 'SELECT SUM( project_budget.amount ) as cat_budget FROM project_budget JOIN budget_items ON project_budget.item_id = budget_items.item_id WHERE budget_items.parent_item_id =' . $cat_id . ' AND project_budget.project_id =' . $project_id;
        //echo $stmt;
        $rs = $conn->ExecuteNonQuery($stmt);
        $row = mysql_fetch_array($rs);
        return $row['cat_budget'];
    }
}
