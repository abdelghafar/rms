...
//    $pdf->SetFont('aealarabiya', '', 14);
    ...
    $html = $html_styles . '<p class="ph1">' . 'Project team' . '</p>';
    $html .='<hr/><br/>';
    ...
    <thead>
        <tr>
            ...
            <th style="width: 300px;" >Member Name</th>
    ...
    while ($row = mysql_fetch_array($rs)) {
        if ($row['type'] === 'role_based') {
            $person_name = $row['role_name_en'];
            $role_name = '';
            $Position = '';
        } else {
            $person_name = $person->GetPersonNameEn($row['person_id']);
            $Position = $person->GetPersonPosition($row['person_id']);
            $role_name = $row['role_name_en'];
        }
        if ($td_style == "tg-odd")
            $td_style = "tg-even";
        else
