    ...
    (CONNECT_DATA =
      ...
      (SERVICE_NAME = PROD1)
        ...
        if (!$conn) {
            ...
            $stid = oci_parse($conn, 'SELECT * FROM vrsh_instructors WHERE EMPLOYEE_ID = 4361078');


            oci_execute($stid);
            
            $imported_emp_count = 0;
            
            while (($row = oci_fetch_array($stid, OCI_ASSOC))) {
                $time = strtotime($row['RANK_DATE']);
                $rank_date = date('y-m-d', $time);

                //$rank_date = date_format($row['RANK_DATE'], 'DD-MM-YYYY')
